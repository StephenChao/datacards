...
ccargs_VH="VHTopCR=${cards_dir}/VH_TopCR.txt VHSR=${cards_dir}/VH_SR.txt "
...
if [ $bfit = 1 ]; then
    ...
    combine -M MultiDimFit  -m 125 -d ${wsm}.root --rMax ${rMax} --rMin ${rMin} \
    --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance ${mintol}  \
    -n SnapshotObserved --algo grid --points ${npoints}  2>&1 | tee $outsdir/ObservedMultiDimFit.txt
    # secondly use the breakdown
    combine -M MultiDimFit  -m 125 -d ${wsm}.root --rMax ${rMax} --rMin ${rMin} \
    --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance ${mintol}  \
    ...
    -m 125 --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance ${mintol} \
    --rMax ${rMax} --rMin ${rMin} --algo grid --points ${npoints} \
...
if [ $impactsi = 1 ]; then
    ...
    --expectSignal 1 --rMin -3 --rMax 2 \
    -d $wsm.root --doInitialFit --robustFit 1 \
    --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 0.5 2>&1 | tee $outsdir/Impacts_init.txt
fi
...
if [ $impactsf != 0 ]; then
    ...
    --rMin -6 --rMax 4 --cminDefaultMinimizerStrategy 0  --cminDefaultMinimizerTolerance 3 -m 125 | tee "$outsdir/Impacts_$impactsf.txt"
fi
...
if [ $impactsc != 0 ]; then
    ...
    --rMin -6 --rMax 4 --cminDefaultMinimizerStrategy 0  --cminDefaultMinimizerTolerance 3 -v 1 -o impacts.json 2>&1 | tee $outsdir/Impacts_collect.txt
